...
```bash
sam-order-checkout$ sam logs -n StockCheckerFunction --stack-name sam-order-checkout --tail
```
...
```bash
sam-order-checkout$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name sam-order-checkout
```
